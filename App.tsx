    ...
    return (
      <div className="min-h-screen bg-slate-100 flex flex-col items-center justify-center p-6">
        <div className="w-full max-w-xl mb-12 flex justify-between items-center bg-white p-4 rounded-3xl shadow-sm border border-slate-200">
           ...
           <button onClick={() => setShowSyncModal(true)} className="text-blue-600 text-xs font-bold bg-blue-50 px-4 py-2 rounded-xl flex items-center gap-2">
        ...
        <div className="text-center mb-10 animate-slide-up">
          ...
          <p className="text-slate-500 text-xs mt-2 tracking-widest uppercase">Hospital Food Tracking</p>
        </div>
              ...
              <input 
                ...
                onKeyDown={(e) => {
                  if (e.key === 'Enter') {
                    ...
                    if (val) { setRoomId(val); localStorage.setItem('hft_room_id', val); setShowSyncModal(false); pullFromCloud(); }
                  }
                }}
              />
              <button onClick={() => {
                const val = (document.querySelector('input') as HTMLInputElement).value.toUpperCase().trim();
                if (val) { setRoomId(val); localStorage.setItem('hft_room_id', val); setShowSyncModal(false); pullFromCloud(); }
              }} className="w-full bg-blue-600 text-white font-bold py-5 rounded-[2rem] shadow-xl">ตกลง</button>
            </div>
          </div>
        )}
      </div>
    ...
    <div className="min-h-screen bg-slate-50 pb-20">
      <header className="glass border-b border-slate-200 px-6 py-4 flex justify-between items-center sticky top-0 z-40">
        <div className="flex items-center gap-3">
          ...
          <div><div className="font-extrabold text-slate-900 leading-none">HFT PRO</div><div className="text-[10px] font-bold text-slate-400 mt-1 uppercase">{role}</div></div>
        </div>
        <div className="flex items-center gap-2">
           <div className={`px-3 py-1.5 rounded-full text-[10px] font-bold border ${isSyncing ? 'bg-blue-50 text-blue-600 border-blue-100' : 'bg-emerald-50 text-emerald-600 border-emerald-100'}`}>
             {isSyncing ? 'SYNCING...' : roomId}
           </div>
           <button onClick={() => saveRole(null)} className="p-2.5 bg-white border border-slate-200 rounded-xl text-slate-400 hover:text-rose-500 shadow-sm"><LogOut className="w-5 h-5" /></button>
        </div>
      </header>
...
const RoleCard = ({ icon, title, desc, color, onClick }: any) => (
  <button onClick={onClick} className="bg-white p-6 rounded-[2.5rem] shadow-sm border border-slate-200/50 flex items-center gap-6 text-left hover:shadow-xl group transition-all">
    ...
    <div className="flex-1"><h3 className="text-xl font-bold text-slate-900 mb-1">{title}</h3><p className="text-slate-400 text-xs">{desc}</p></div>
