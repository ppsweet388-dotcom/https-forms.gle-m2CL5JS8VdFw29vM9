    ...
    return (
      <div className="min-h-screen bg-[#f1f5f9] flex flex-col items-center justify-center p-6 bg-[radial-gradient(#cbd5e1_1px,transparent_1px)] [background-size:24px_24px]">
        <div className="w-full max-w-md space-y-6 text-center animate-in fade-in zoom-in duration-500">
          ...
          <p className="text-[10px] text-slate-400 font-bold uppercase tracking-[0.4em]">Hospital Logistics Network</p>
          ...
          {wardCode && (
            <div className={`p-5 rounded-[2.5rem] flex items-center justify-between shadow-2xl border-4 border-white ${onlineStatus ? 'bg-blue-600' : 'bg-red-500'} text-white transition-all`}>
              <div className="flex items-center gap-4 text-left">
                ...
                <div>
                  <p className="text-[9px] font-black opacity-60 uppercase tracking-widest leading-none mb-1">Current Station</p>
      ...
      <main className="p-4 max-w-2xl mx-auto w-full space-y-6">
        {activeRole !== 'VIEWER' && (
          <div className="grid grid-cols-4 gap-2">
            ...
            <StatCard label="เสร็จสิ้น" value={stats.done} color="text-green-600" />
          </div>
        )}
        ...
        {activeRole === 'VIEWER' && (
          <div className="bg-white p-8 rounded-[3rem] shadow-xl border border-slate-100 space-y-8">
            <div className="space-y-2 text-left">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Ward Identity</label>
            ...
            <div className="grid grid-cols-1 gap-4">
              <button onClick={cloudId ? () => pushToCloud() : handleInitialSetup} className="w-full py-6 bg-blue-600 text-white rounded-[2rem] font-black shadow-lg uppercase italic active:scale-[0.98] transition-all">
                {cloudId ? 'บันทึก & ซิงค์ข้อมูล' : 'เปิดใช้งาน Cloud Sync'}
              </button>
              {cloudId && <button onClick={() => setShowQr(true)} className="w-full py-6 bg-slate-900 text-white rounded-[2rem] font-black shadow-lg uppercase italic flex items-center justify-center gap-2 active:scale-[0.98] transition-all"><QrIcon className="w-5 h-5"/> โชว์ QR แชร์วอร์ด</button>}
            </div>
          </div>
        )}

        {activeRole === 'ADMIN' && (
          <button onClick={() => setShowOrderForm(true)} className="w-full py-10 bg-white border-4 border-dashed border-blue-100 rounded-[3rem] flex flex-col items-center gap-2 text-blue-600 font-black hover:bg-blue-50 transition-all active:scale-[0.98]">
            <Plus className="w-8 h-8" />
            <span className="text-xl italic uppercase tracking-tighter">สร้างใบสั่งใหม่</span>
          </button>
        )}
        ...
        {activeRole !== 'VIEWER' && (
          <div className="relative">
            ...
            <input type="text" placeholder="ค้นหาชื่อ, HN, ห้อง..." className="w-full pl-16 pr-8 py-6 bg-white rounded-[2.2rem] border-none font-bold outline-none shadow-sm focus:ring-4 ring-blue-50 transition-all text-lg" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
          </div>
        )}
        ...
        <div className="space-y-3">
          ...
          {filteredLogs.length === 0 && <div className="py-20 text-center opacity-30 italic font-black uppercase tracking-widest">ไม่มีข้อมูล</div>}
        </div>
      </main>
      ...
      {showQr && (
        <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-xl z-[200] flex items-center justify-center p-6" onClick={() => setShowQr(false)}>
          <div className="bg-white p-10 rounded-[4rem] text-center space-y-8 w-full max-w-sm border-8 border-white animate-in zoom-in-95" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex justify-center p-6 bg-slate-50 rounded-[3rem] shadow-inner" ref={el => {
              if (el && !el.innerHTML) {
                // @ts-ignore
            ...
            <button onClick={() => setShowQr(false)} className="w-full py-6 bg-slate-900 text-white font-black rounded-[2rem] uppercase italic">ปิด</button>
          </div>
        </div>
      )}
    ...
    <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-md z-[100] p-4 flex items-center justify-center">
      <div className="bg-white w-full max-w-md rounded-[3.5rem] shadow-2xl overflow-hidden flex flex-col max-h-[90vh] border-8 border-white animate-in slide-in-from-bottom-20">
        <div className="p-8 bg-blue-600 text-white flex justify-between items-center">
          <h3 className="font-black text-2xl italic uppercase tracking-tighter">New Meal Order</h3>
          <button onClick={onClose} className="p-3 bg-white/10 rounded-full active:scale-90"><X /></button>
        </div>
        <div className="p-8 space-y-4 overflow-y-auto no-scrollbar bg-slate-50/50 text-left">
          ...
          <div className="space-y-1">
            ...
            <textarea placeholder="ระบุรายการอาหาร..." className="w-full p-4 bg-white rounded-2xl h-24 border border-slate-100 outline-none focus:ring-4 ring-blue-50 shadow-inner" value={f.menuItems} onChange={e=>setF({...f, menuItems:e.target.value})}/>
          </div>
      ...
      <div className="bg-white w-full max-w-md rounded-[4rem] shadow-2xl overflow-hidden border-8 border-white animate-in zoom-in-95" onClick={e => e.stopPropagation()}>
        <div className="p-10 space-y-8 text-left overflow-y-auto max-h-[90vh] no-scrollbar">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h4 className="text-[10px] font-black uppercase text-slate-400 italic">รูปถ่าย / ไฟล์แนบ</h4>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              {(!log.attachments || log.attachments.length === 0) && <div className="col-span-2 py-6 border-2 border-dashed border-slate-100 rounded-[2rem] text-center opacity-20 text-[9px] font-black uppercase tracking-widest italic">No Attachments</div>}
            </div>
          </div>
          ...
          <div className="pt-2">
            {role === 'KITCHEN' && log.status === MealStatus.ORDERED && (
              <div className="grid grid-cols-2 gap-3">
                <button onClick={() => onUpdate(log.id, MealStatus.KITCHEN_READY, 'Kitchen Staff')} className="py-6 bg-orange-500 text-white font-black text-xl rounded-2xl shadow-xl uppercase italic active:scale-95 transition-all">Ready</button>
                <button onClick={() => onShowLabel(log)} className="py-6 bg-slate-900 text-white font-black text-xl rounded-2xl flex items-center justify-center gap-2 uppercase italic shadow-xl active:scale-95 transition-all"><Printer className="w-5 h-5"/> Label</button>
              </div>
            )}
            {role === 'DISPATCH' && log.status === MealStatus.KITCHEN_READY && <button onClick={() => onUpdate(log.id, MealStatus.DISPATCHED, 'Courier Staff')} className="w-full py-7 bg-indigo-600 text-white font-black text-2xl rounded-2xl shadow-xl uppercase italic active:scale-95 transition-all">Pick up</button>}
            {role === 'DELIVERY' && log.status === MealStatus.DISPATCHED && <button onClick={() => onUpdate(log.id, MealStatus.DELIVERED, 'Service Staff')} className="w-full py-7 bg-green-600 text-white font-black text-2xl rounded-2xl shadow-xl uppercase italic active:scale-95 transition-all">Serve</button>}
          </div>
        </div>
      </div>
    </div>
...
const LabelPrint = ({ log, onClose }: any) => (
  <div className="fixed inset-0 bg-white z-[300] flex flex-col items-center justify-center p-8">
    ...
    <div className="mt-12 flex gap-4 no-print">
      <button onClick={() => window.print()} className="px-10 py-5 bg-blue-600 text-white font-black rounded-2xl flex items-center gap-3 shadow-xl uppercase italic active:scale-95 transition-all"><Printer className="w-6 h-6"/> Print</button>
      <button onClick={onClose} className="px-10 py-5 bg-slate-100 font-black rounded-2xl uppercase italic active:scale-95 transition-all">Back</button>
    </div>
  </div>
);
